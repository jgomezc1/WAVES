...
nini = nfin
nfin , els2_array  = msw.ele_writer(cells , cell_data , 'quad9' , 12000 , 3 , 5 , 18 , 9 , nini )
nini = nfin
nfin , els3_array  = msw.ele_writer(cells , cell_data , 'quad9' , 13000 , 3 , 6 , 18 , 9 , nini )
nini = nfin
nfin , els4_array  = msw.face_recognition(cells , cell_data , 10000 , 5000 , nini)
nini = nfin
nfin , els5_array  = msw.boundary_writer(cells , cell_data , 7000 , 7 , nini)
#
#%%
#
...
np.savetxt("9eles.txt", els5_array   , fmt="%d")


file_list = glob.glob("*.txt")

with open('layer.inp', 'w') as file:
