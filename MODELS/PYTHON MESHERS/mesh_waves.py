...
def face_recognition(cells , cell_data , phy_sur , phy_lin , nini):
    ...
    elements1 = cells["quad9"] + 1            # Element connectivities (adds 1 to fortranize)  
    ...
    els1_array[: , 0] = range(1 , len(strip_id) + 1 + nini)
    els1_array[: , 1] = 5
    els1_array[: , 2] = 18
    els1_array[: , 4] = 9
...
def ele_writer(cells , cell_data , ele_tag , phy_sur , ele_type , mat_tag , ndof , nnode , nini):
    ...
    elements = cells[ele_tag]           # Element connectivities (adds 1 to fortranize)
...
def node_writer(points , point_data):
    ...
    nodes_array[:, 4:6] = points[:, :2]       # Nodal coordinates (starting in column 2 from the .msh file into columns 4 and 5 of the output file).
    nodes_array[nodes_array[:, 1]==0, 1] = 2  # Number of degrees of freedom per node.
