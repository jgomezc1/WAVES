...
!
!ND=ND-NIMPRIME
NB=ND-NEL-NIMPRIME-NELINCO-NELINCOM

ALLOCATE(NODEIZ(NB), NODECEN(NB), &
         NODEDER(NB))
...
ALLOCATE(NODEIZINCO(NELINCO), NODECENINCO(NELINCO), &
         NODEDERINCO(NELINCO))
...
MM=0
!
DO I=1, NB+NIMPRIME+NELINCO+NELINCOM
...
DO I=1, NB
    ...
    DO J=1, NEL
        ...
        IF (NX==MIE(1,J) .AND. NY==MIE(2,J) .AND. INCO1(J,2)==11) THEN
        ...
        IF (NX==MIE(2,J) .AND. NY==MIE(3,J) .AND. INCO1(J,2)==11) THEN
        ...
        IF (NX==MIE(3,J) .AND. NY==MIE(4,J) .AND. INCO1(J,2)==11) THEN
        ...
        IF (NX==MIE(4,J) .AND. NY==MIE(1,J) .AND. INCO1(J,2)==11) THEN
...
ALLOCATE(NODESDRMMOD(NELINCOM,9))
NODESDRMMOD=0
...
DO I=1, NELINCOM
    ...
    DO J=1, NEL
        ...
        IF (NX==MIE(1,J) .AND. NY==MIE(2,J) .AND. INCO1(J,2)==15) THEN

            NODESDRMMOD(I,:)=MIE(:,J)
!             NODESDRMMOD(I,1)=MIE(1,J)
!             NODESDRMMOD(I,2)=MIE(2,J)
!             NODESDRMMOD(I,3)=MIE(3,J)
!             NODESDRMMOD(I,4)=MIE(4,J)
!             NODESDRMMOD(I,5)=MIE(5,J)
!             NODESDRMMOD(I,6)=MIE(6,J)
!             NODESDRMMOD(I,7)=MIE(7,J)
!             NODESDRMMOD(I,8)=MIE(8,J)
!             NODESDRMMOD(I,9)=MIE(9,J)
            EXIT

        END IF

        IF (NX==MIE(2,J) .AND. NY==MIE(3,J) .AND. INCO1(J,2)==15) THEN
...
!             NODESDRMMOD(I,9)=MIE(9,J)
        ...
        IF (NX==MIE(3,J) .AND. NY==MIE(4,J) .AND. INCO1(J,2)==15) THEN
...
!             NODESDRMMOD(I,9)=MIE(9,J)
        ...
        IF (NX==MIE(4,J) .AND. NY==MIE(1,J) .AND. INCO1(J,2)==15) THEN
...
DEALLOCATE(IMPRIME1)
ALLOCATE(IMPRIME1(NELINCOM,9))
IMPRIME1=NODESDRMMOD
...
DO J=1, 4
    ...
    DO I=1, NELINCOM-1
        IPOS=0
        IFLAG=1
        IF (IMPRIME1(I,1).NE.0) THEN
            DO WHILE (IFLAG==1)
                CALL BELONGS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,1),IFLAG)
                IF (IFLAG==1) THEN
                    CALL SEARCHPOS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,1),IPOS)
                    IMPRIME1(IPOS,J)=0
                END IF
            END DO
        END IF
...
DO J=2, 4
    ...
    DO I=1, NELINCOM-1
        ...
        IF (IMPRIME1(I,2).NE.0) THEN
            DO WHILE (IFLAG==1)
                CALL BELONGS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,2),IFLAG)
                IF (IFLAG==1) THEN
                    CALL SEARCHPOS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,2),IPOS)
                    IMPRIME1(IPOS,J)=0
                END IF
            END DO
        END IF
...
DO J=3, 4
    ...
    DO I=1, NELINCOM-1
        IFLAG=1
        IPOS=0
        IF (IMPRIME1(I,3).NE.0) THEN
            DO WHILE (IFLAG==1)
                CALL BELONGS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,3),IFLAG)
                IF (IFLAG==1) THEN
                    CALL SEARCHPOS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,3),IPOS)
                    IMPRIME1(IPOS,J)=0
                END IF
            END DO
        END IF
...
DO I=1, NELINCOM-1
    IFLAG=1
    IPOS=0
    IF (IMPRIME1(I,4).NE.0) THEN
        DO WHILE (IFLAG==1)
            CALL BELONGS(IMPRIME1(:,4),NELINCOM,NODESDRMMOD(I+1,4),IFLAG)
            IF (IFLAG==1) THEN
                CALL SEARCHPOS(IMPRIME1(:,4),NELINCOM,NODESDRMMOD(I+1,4),IPOS)
                IMPRIME1(IPOS,4)=0
            END IF
        END DO
    END IF
...
DO J=5, 8
    ...
    DO I=1, NELINCOM-1
        IFLAG=1
        IPOS=0
        IF (IMPRIME1(I,5).NE.0) THEN
            DO WHILE (IFLAG==1)
                CALL BELONGS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,5),IFLAG)
                IF (IFLAG==1) THEN
                    CALL SEARCHPOS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,5),IPOS)
                    IMPRIME1(IPOS,J)=0
                END IF
            END DO
        END IF
...
DO J=6, 8
    ...
    DO I=1, NELINCOM-1
        IFLAG=1
        IPOS=0
        IF (IMPRIME1(I,6).NE.0) THEN
            DO WHILE (IFLAG==1)
                CALL BELONGS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,6),IFLAG)
                IF (IFLAG==1) THEN
                    CALL SEARCHPOS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,6),IPOS)
                    IMPRIME1(IPOS,J)=0
                END IF
            END DO
        END IF
...
DO J=7, 8
    ...
    DO I=1, NELINCOM-1
        IFLAG=1
        IPOS=0
        IF (IMPRIME1(I,7).NE.0) THEN
            DO WHILE (IFLAG==1)
                CALL BELONGS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,7),IFLAG)
                IF (IFLAG==1) THEN
                    CALL SEARCHPOS(IMPRIME1(:,J),NELINCOM,NODESDRMMOD(I+1,7),IPOS)
                    IMPRIME1(IPOS,J)=0
                END IF
            END DO
        END IF
...
DO I=1, NELINCOM-1
    IFLAG=1
    IPOS=0
    IF (IMPRIME1(I,8).NE.0) THEN
        DO WHILE (IFLAG==1)
            CALL BELONGS(IMPRIME1(:,8),NELINCOM,NODESDRMMOD(I+1,8),IFLAG)
            IF (IFLAG==1) THEN
                CALL SEARCHPOS(IMPRIME1(:,8),NELINCOM,NODESDRMMOD(I+1,8),IPOS)
                IMPRIME1(IPOS,8)=0
            END IF
        END DO
    END IF
...
WRITE(10, '(3I8, 1F8.3, 8I6)') NP, NEL+NB, NMAT+7, TTOT, NT, 2, &
                               9, 18, 5, 1, 0, NIMPRIME!+KK
...
!
!
!PRIMER DATO CORRESPONDE AL SEMIESPACIO
!
...
DO I=1, NEL
!
    IF (MIE(9,I)==122222 .OR. MIE(9,I)==125804) THEN
        WRITE(10,'(1I7, 4I4, 9I8)') I, INCO1(I,1), 18, 6, 9, MIE(1,I), MIE(2,I), &
